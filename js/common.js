...
//            domArr[domArr.length-1].search(knownDomsPat)==-1) {
     ...
     * 숫자에 comma를 제거한다.      
     */
    function priceToStringDel(price) {
...
//		ls_pri = "toolbar=no, location=no, directories=no, menubar=no, resizable=yes, scrollbars="+s_scroll+", status=yes width="+s_width+" height="+s_height;
			...
			jQuery(this).datepicker({
				...
				beforeShow: function() {
					...
					if(clearYn == "Y"){
	  				...
	  				/*if(inst.id == "p_edu_start_dt" || inst.id == "p_edu_end_dt") {
						classHistoryCountdraw();
					}*/
	            }  
			});
		})
  ...
  //KMA 멤버십 리뉴얼(20230822_포도소프트웨어 김유진)
  //팝업열기
  function openPopup(popupId){
	  var popupHeight = $("#"+popupId).height();	//팝업높이 
	  
	  $(".popup-layer").show();
	  $("#"+popupId).css('top', (window.innerHeight-popupHeight)/2);
	  $("#"+popupId).show();
  }
  //팝업닫기
  function closePopup(popupId){
	  ...
	  $("#"+popupId+" input[type=radio]").prop("checked", false);
